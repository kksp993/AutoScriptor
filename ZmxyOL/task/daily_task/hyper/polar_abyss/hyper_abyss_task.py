...
def task(
    ...
    ZhangWang: JhsyNandu = JhsyNandu.灵域,
    HuWang: JhsyNandu = JhsyNandu.灵域,
    LangWang: JhsyNandu = JhsyNandu.灵域,
    QuanShen: JhsyNandu = JhsyNandu.灵域,
    YanHao: JhsyNandu = JhsyNandu.灵域,
    ...
    **kwargs,
    ...
    if set(nandu_map) != set(JHSY_CHAOS_TABLE):
        logger.error("JHSY_CHAOS_TABLE 与 hyper_abyss_task 参数映射不一致，请同步修改")
        return

    def _phase2_expect_difficulty(nandu: JhsyNandu) -> str:
        # 灵域只用于灵狱匹配候选，二阶段按噩梦打（灵狱次数已在阶段一消耗）
    ...
    lingyu_candidates = [n for n in JHSY_CHAOS_TABLE if nandu_map[n] == JhsyNandu.灵域]
    # 未配置任何「灵域」时与旧版一致：全表参与匹配；否则只遍历用户勾选的灵域副本
