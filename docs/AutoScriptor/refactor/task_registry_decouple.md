...
| 数据 | 存储位置 | 是否持久化 | 说明 |
|------|----------|-----------|------|
| `on` `next_exec_time` `params` `next_exec_offset_hours` | `cfg["tasks"]` | 是（JSON） | 用户可编辑配置 |
| `fn` `order` `param_meta` | `TaskRegistry` | 否（内存） | 代码定义的运行时数据 |
...
| 方法 | 签名 | 说明 |
|------|------|------|
| `register` | `(path: str, fn, order: int, param_meta: dict \| None = None)` | 注册/覆盖一个任务 |
...
```
@register_task
    ├── cfg["tasks"] ← { on, next_exec_time, params, next_exec_offset_hours }
    └── TaskRegistry ← { fn, order, param_meta }
...
ActionHandler.do_param_edit(node, path)
    └── task_registry.get_param_meta(path) → 获取枚举类型信息
```
...
| `AutoScriptor/utils/constant.py` | `_clean_tasks_for_saving` 保留为防御性清理 |
...
```powershell
# 在项目 conda 环境中运行
conda run -n zmxy python -m unittest discover -s test/test_task_registry -v
```
...
- **向后兼容**：`_clean_tasks_for_saving` 仍保留 `fn`/`order` 的防御性清理，兼容过渡期
