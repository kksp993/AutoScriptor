...
- Same-line update zips that advance the recorded release version must also replace the installed daily launcher `造笔.exe` with the current portable installer via `--include-file`. Otherwise the old launcher may open its old installer wizard after update and never start the updated backend.
...
- For paths, use `AutoScriptor.utils.paths`; source mode and packaged mode intentionally differ (`logs/` vs data-root `logs/`, root `config.json` vs packaged `install.json.dataRoot/config.json`).
