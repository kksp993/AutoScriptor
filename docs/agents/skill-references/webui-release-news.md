...
- If VM headless install reports `backend 校验失败：缺少 autoscriptor-engine.exe` immediately after `tar.exe 解压完成`, treat native tar success as untrusted. Verify the staging backend after tar, and fall back to JS extraction when the staging tree is invalid.
- If a VM same-line update passes apply/direct-backend checks but the launcher opens the wrong install path or old wizard, inspect JSON marker encoding. Guest PowerShell 5.1 must write `install.json` and `.autoscriptor/release_version.json` as UTF-8 without BOM; `Set-Content -Encoding UTF8` can add a BOM that breaks Node `JSON.parse`.
...
- VM install/update acceptance is the release gate for WebUI/Electron changes too. Local smoke, `verify-pack`, and update dry-run are required prechecks, but do not push the release commit/tag or publish artifacts until the VM report JSON/logs for the current installer/update package have been reviewed.
