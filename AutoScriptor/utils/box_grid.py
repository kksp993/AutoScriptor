...
def _is_box_like(box) -> bool:
    return isinstance(box, Box) or (hasattr(box, "box") and isinstance(box.box, Box))
...
def indexof(box_grid, target_box) -> tuple[int, int] | None | list[tuple[int, int] | None]:
    """Return the (row, col) index of the cell that fully contains target_box."""
    is_single = _is_box_like(target_box) or target_box is None
    target_boxes = [target_box] if is_single else target_box
    indices: list[tuple[int, int] | None] = [None] * len(target_boxes)

    for i, target in enumerate(target_boxes):
        if target is None:
            continue
        target = _as_box(target)
        for r, row in enumerate(box_grid):
            for c, cell in enumerate(row):
                if target.is_in(_as_box(cell)):
                    indices[i] = (r, c)
                    break
